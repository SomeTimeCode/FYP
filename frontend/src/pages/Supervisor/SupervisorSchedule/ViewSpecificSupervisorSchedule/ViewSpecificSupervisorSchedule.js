...
import './ViewSpecificSupervisorSchedule.css'
    ...
    MySwal.fire({
      ...
      if(result.isConfirmed){
        MySwal.fire({
          ...
          temp[date][time]['commit'] = (result.value === ''? `${props.supervisor} ${props.data[date][time]["available"]? "is unavailable" : "removes unavailable"}` : result.value)
          props.setData(temp)
        }).then(() => {
          MySwal.fire({
            title: "Updated"
      ...
      <tbody>
        {
          timeslot.map((time)=>{
            return (
              <Row key={time} time={time} data={props.data} setData={props.setData} supervisor={props.supervisor}/>
            )
          })
        }
      </tbody>
...
function ViewSpecificSupervisorSchedule() {
  ...
  useEffect(() => {
      const fetchData = async () => {
          ...
          let response = await fetch(`${process.env.REACT_APP_BACKEND_URI}/api/supervisor/viewSpecificSchedule/${params.id}`, requestOptions)
  ...
  const sumbit = (event) => {
    ...
    MySwal.fire({
      ...
      if(result.isConfirmed){
        ...
        let response = await fetch(`${process.env.REACT_APP_BACKEND_URI}/api/supervisor/updateSpecificSchedule`, requestOptions)
  ...
  return (
    <div id='ViewSpecificSupervisorScheduleBase'>
            {loading?
                <>fetching data</>
            :
            <>
              <div id='title'>
                {schedule.term} Presentation Schedules
              </div>
              <div id='info'>
                <div id='Schedules'>
                  <Table data={data} setData={setData} supervisor={supervisor}/>
                </div>
...
export default ViewSpecificSupervisorSchedule